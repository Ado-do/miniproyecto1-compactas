...
public:
    OccBalancedWT() = delete;
    OccBalancedWT(const std::vector<uint8_t> &seq) {
        std::string s(seq.begin(), seq.end());
        construct_im(wt, s, 1);
    }
    uint64_t occ(uint8_t v, uint64_t k) { return wt.rank(k, v); }
    double size_mb() const {
        size_t bytes = sizeof(OccBalancedWT) + sdsl::size_in_bytes(wt);
...
public:
    ...
    uint64_t occ(uint8_t v, uint64_t k) { return wt.rank(k, v); }
    double size_mb() const {
        size_t bytes = sizeof(OccHuffmanWT) + sdsl::size_in_bytes(wt);
        return bytes / (1024.0 * 1024.0);
    }
};
